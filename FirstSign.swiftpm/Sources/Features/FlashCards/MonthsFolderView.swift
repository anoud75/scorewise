...
struct MonthsFolderView: View {
    @ObservedObject var viewModel: MonthsFolderViewModel
    let onClose: () -> Void
    ...
    private var baseWindow: some View {
        Group {
            if let image = AssetImageLoader.image(named: "months-window-base") {
                Image(uiImage: image)
                    .resizable()
                    .scaledToFill()
            } else {
    ...
    private var titleBar: some View {
        ZStack {
            if let image = AssetImageLoader.image(named: "months-title-bar") {
                Image(uiImage: image)
                    .resizable()
                    .scaledToFill()
            } else {
            ...
            HStack(spacing: 10) {
                if let icon = AssetImageLoader.image(named: "months-title-icon") {
                    Image(uiImage: icon)
                        .resizable()
                        .scaledToFit()
                ...
                Button(action: onClose) {
                    if let closeImage = AssetImageLoader.image(named: "months-close-button") {
    ...
    private var scrollBar: some View {
        ZStack {
            if let image = AssetImageLoader.image(named: "months-scroll-bar") {
                Image(uiImage: image)
                    .resizable()
                    .scaledToFill()
            } else {
    ...
    private func monthCard(_ item: MonthCardItem) -> some View {
        Group {
    ...
    private func cardPreviewOverlay(_ item: MonthCardItem) -> some View {
        ZStack(alignment: .topTrailing) {
            ...
            VStack(spacing: 0) {
                ...
                .frame(maxWidth: 560, maxHeight: 860)
...
#Preview {
    MonthsFolderView(viewModel: MonthsFolderViewModel(), onClose: {})
}
