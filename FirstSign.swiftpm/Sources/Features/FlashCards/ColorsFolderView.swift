...
struct ColorsFolderView: View {
    @ObservedObject var viewModel: ColorsFolderViewModel
    let onClose: () -> Void
    ...
    private var baseWindow: some View {
        Group {
            if let image = AssetImageLoader.image(named: "colors-window-base") {
                Image(uiImage: image)
                    .resizable()
                    .scaledToFill()
            } else {
    ...
    private var titleBar: some View {
        ZStack {
            if let image = AssetImageLoader.image(named: "colors-title-bar") {
                Image(uiImage: image)
                    .resizable()
                    .scaledToFill()
            } else {
            ...
            HStack(spacing: 10) {
                if let icon = AssetImageLoader.image(named: "colors-title-icon") {
                    Image(uiImage: icon)
                        .resizable()
                        .scaledToFit()
                ...
                Button(action: onClose) {
                    if let closeImage = AssetImageLoader.image(named: "colors-close-button") {
    ...
    private var scrollBar: some View {
        ZStack {
            if let image = AssetImageLoader.image(named: "colors-scroll-bar") {
                Image(uiImage: image)
                    .resizable()
                    .scaledToFill()
            } else {
    ...
    private func colorCard(_ item: ColorCardItem) -> some View {
        Group {
    ...
    private func cardPreviewOverlay(_ item: ColorCardItem) -> some View {
...
#Preview {
    ColorsFolderView(viewModel: ColorsFolderViewModel(), onClose: {})
}
