...
struct NumbersFolderView: View {
    @ObservedObject var viewModel: NumbersFolderViewModel
    let onClose: () -> Void
    ...
    private var baseWindow: some View {
        Group {
            if let image = AssetImageLoader.image(named: "numbers-window-base") {
                Image(uiImage: image)
                    .resizable()
                    .scaledToFill()
            } else {
    ...
    private var titleBar: some View {
        ZStack {
            if let image = AssetImageLoader.image(named: "numbers-title-bar") {
                Image(uiImage: image)
                    .resizable()
                    .scaledToFill()
            } else {
            ...
            HStack(spacing: 10) {
                if let icon = AssetImageLoader.image(named: "numbers-title-icon") {
                    Image(uiImage: icon)
                        .resizable()
                        .scaledToFit()
                ...
                Button(action: onClose) {
                    if let closeImage = AssetImageLoader.image(named: "numbers-close-button") {
    ...
    private var scrollBar: some View {
        ZStack {
            if let image = AssetImageLoader.image(named: "numbers-scroll-bar") {
                Image(uiImage: image)
                    .resizable()
                    .scaledToFill()
            } else {
    ...
    private func numberCard(_ item: NumberCardItem) -> some View {
        Group {
        ...
        .aspectRatio(0.895, contentMode: .fit)
    }

    private func cardPreviewOverlay(_ item: NumberCardItem) -> some View {
        ZStack(alignment: .topTrailing) {
            ...
            VStack(spacing: 0) {
                ...
                .frame(maxWidth: 620, maxHeight: 860)
...
#Preview {
    NumbersFolderView(viewModel: NumbersFolderViewModel(), onClose: {})
}
