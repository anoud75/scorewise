...
struct EmotionsFolderView: View {
    @ObservedObject var viewModel: EmotionsFolderViewModel
    let onClose: () -> Void
    ...
    private var baseWindow: some View {
        Group {
            if let image = AssetImageLoader.image(named: "emotions-window-base") {
                Image(uiImage: image)
                    .resizable()
                    .scaledToFill()
            } else {
    ...
    private var titleBar: some View {
        ZStack {
            if let image = AssetImageLoader.image(named: "emotions-title-bar") {
                Image(uiImage: image)
                    .resizable()
                    .scaledToFill()
            } else {
            ...
            HStack(spacing: 10) {
                if let icon = AssetImageLoader.image(named: "emotions-title-icon") {
                    Image(uiImage: icon)
                        .resizable()
                        .scaledToFit()
                ...
                Button(action: onClose) {
                    if let closeImage = AssetImageLoader.image(named: "emotions-close-button") {
    ...
    private var scrollBar: some View {
        ZStack {
            if let image = AssetImageLoader.image(named: "emotions-scroll-bar") {
                Image(uiImage: image)
                    .resizable()
                    .scaledToFill()
            } else {
    ...
    private func emotionCard(_ item: EmotionCardItem) -> some View {
        Group {
    ...
    private func cardPreviewOverlay(_ item: EmotionCardItem) -> some View {
...
#Preview {
    EmotionsFolderView(viewModel: EmotionsFolderViewModel(), onClose: {})
}
